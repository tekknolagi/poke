      ...
      if (add_new_percent_arg_p || add_new_style_arg_p)
        {
          ...
          new_arg = pkl_ast_make_print_stmt_arg (PKL_PASS_AST, NULL);
          PKL_AST_LOC (new_arg) = PKL_AST_LOC (print_fmt);

          if (add_new_percent_arg_p)
            PKL_AST_PRINT_STMT_ARG_SUFFIX (new_arg) = xstrdup ("%");
          else if (add_new_style_arg_p)
            {
              ...
              assert (new_style_class != NULL);
              if (end_sc)
                PKL_AST_PRINT_STMT_ARG_END_SC (new_arg) = new_style_class;
              else
                PKL_AST_PRINT_STMT_ARG_BEGIN_SC (new_arg) = new_style_class;
              ...
              if (arg == PKL_AST_PRINT_STMT_ARGS (print_stmt))
                {
                  ...
                  PKL_AST_PRINT_STMT_ARGS (print_stmt) = ASTREF (new_arg);
                }
              else
                {
                  /* Add after.  */
          ...
          else
            {
              ...
              if (!PKL_AST_PRINT_STMT_ARGS (print_stmt))
                ...
                PKL_AST_PRINT_STMT_ARGS (print_stmt)
                  = pkl_ast_chainon (PKL_AST_PRINT_STMT_ARGS (print_stmt), new_arg);
            }
